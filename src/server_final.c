...
void vectorize(cJSON* payload, float* out) {
    ...
    out[1] = clamp((float)installments, MAX_INSTALLMENTS);
    
    float avg_amount = cJSON_GetObjectItem(customer, "avg_amount")->valuedouble;
    float amount_vs_avg = (amount / avg_amount) / AMOUNT_VS_AVG_RATIO;
...
int load_dataset() {
    printf("📦 Gerando %d vetores...\n", MAX_VECTORS);
    g_idx = (VectorIndex*)malloc(sizeof(VectorIndex));
    g_idx->dims = N_DIMS;
    g_idx->n_vectors = MAX_VECTORS;
    ...
    for (int i = 0; i < MAX_VECTORS; i++) {
        ...
        for (int d = 0; d < N_DIMS; d++) {
            temp[d] = (float)rand() / (float)RAND_MAX;
        }
        quantize_int8(temp, &g_idx->vectors[i * N_DIMS], N_DIMS);
        g_idx->labels[i] = (rand() % 5 == 0) ? 1 : 0;
    }
    printf("✅ Dataset: %d vetores\n", g_idx->n_vectors);
    return 0;
}
...
void handle_request(int client_fd, char* body) {
    ...
    for (int i = 0; i < N_NEIGHBORS; i++) {
        if (indices[i] >= 0 && g_idx->labels[indices[i]] == 1) fraud_count++;
    }
...
int main() {
    printf("🚀 Rinha C - Servidor\n");
    if (load_dataset() != 0) return 1;
