...
/* ── Streaming JSON parser for references.json ─────────────── */
/*
 * The file is a JSON array:
 * [{"vector":[f,f,...,f],"label":"fraud"},...]
 * We parse it with a simple state machine.
 */
typedef struct {
    char    *buf;
    size_t   buf_cap;
...
/* Try to parse one record; returns 1 ok, 0 need more data, -1 end/error */
static int parse_record(jbuf_t *j, flat_vec_t *rec) {
    skip_ws(j);
    if (j->pos >= j->buf_len) return 0;
    char c = j->buf[j->pos];
    if (c == ']') return -1; /* end of array */
    ...
    /* Save position to rollback if incomplete */
    size_t saved = j->pos;

    /* Find matching '}' to check we have the whole record */
    ...
    if (depth != 0) { j->pos = saved; return 0; } /* need more data */

    /* Parse: expect "vector" or "label" keys in any order */
...
fail:
    ...
    j->pos++;
    return -2; /* parse error for this record, continue */
}

/* ── K-means clustering ─────────────────────────────────────── */
static void kmeans(float *centroids, const flat_vec_t *vecs,
                   uint32_t n, uint32_t k, int iters) {
    /* Init: pick k random vectors as initial centroids */
    srand(42);
    uint32_t *chosen = malloc(k * sizeof(uint32_t));
    for (uint32_t i = 0; i < k; i++) {
        chosen[i] = (uint32_t)rand() % n;
        memcpy(centroids + i * DIMS, vecs[chosen[i]].v, DIMS * sizeof(float));
    }
    free(chosen);
    ...
    while (!gzeof(gz)) {
        ...
        while (1) {
            ...
            if (r == 1) {
                ...
                goto done_parsing; /* end of array */
            }
            /* r == -2: skip error, continue */
        }
    }
done_parsing:
    gzclose(gz);
    ...
    /* Entries — sorted by centroid */
    /* Build per-centroid sorted write: two-pass */
    ...
    for (uint32_t i = 0; i < g_nvec; i++) {
        ...
        for (int d = 0; d < DIMS; d++) {
            ...
            if (vf == -1.0f) {
                ...
                int16_t qi = (int16_t)(vf * 32767.0f);
                e->v[d] = qi;
            }
        }
    }
